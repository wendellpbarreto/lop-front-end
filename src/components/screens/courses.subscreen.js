...
const CoursesSubScreenComponent = props => {
  ...
  const email = useMemo(
    () => sessionStorage.getItem('user.email').toLowerCase(),
    []
  )
  ...
  const {
    paginedCourses,
    isLoadingCourses,
    isLoadingCoursesToAddInClass,
    paginedCourseToAddInClass,
    getCoursesToAddInClass,
    createCourse,
    updateCourse,
    getCourses,
    getCoursesByClass,
  ...
  const {
    page: pageCoursesToAdd,
    totalPages: totalPagesCoursesToAdd,
    setDocsPerPage: setDocsPerPageCoursesToAdd,
    setPage: setPageCoursesToAdd,
    setTotalPages: setTotalPagesCoursesToAdd,
    setTotalDocs: setTotalDocsCoursesToAdd
  } = usePagination(1, 10)
  ...
  const [titleOrCodeInputCoursesToadd, setTitleOrCodeInputCoursesToadd] =
    useState('')
  ...
  useEffect(() => {
    ...
    paginedCourseToAddInClass,
    setPageCoursesToAdd,
    setTotalDocsCoursesToAdd,
    setDocsPerPageCoursesToAdd,
    setTotalPagesCoursesToAdd
  ])

  const handleCreateNewCourse = useCallback(
    async ({
      currentTitleCourse: title,
      currentDescriptionCourse: description
    }) => {
  ...
  const handleEditCourse = useCallback(
    async (
      id,
      { currentTitleCourse: title, currentDescriptionCourse: description }
    ) => {
  ...
  const handlePageCoursesToAdd = useCallback(
    (e, numPage) => {
      e.preventDefault()
  ...
  return (
    <>
      ...
      <Row>
        ...
        {!paginedCourses
          ? []
          : paginedCourses.docs.map(course => (
              <Col
                className={`col-md-6 ${isLoadingCourses ? 'd-none' : ''}`}
                key={course.id}
              >
                <Card>
                  <CardHead
                        ...
                        <div className="d-flex align-items-center h-100">
                          <div
                            ...
                            style={{
                              ...
                              backgroundImage: `url(${
                                course.author.urlImage || profileImg
                              })`
                            }}
                          />
                          <div
      ...
      >
        <DialogTitle id="contained-modal-title-vcenter">
          {isCreateCourse
            ? 'Criar curso'
            : `Editar curso: ${currentTitleCourse}`}
        </DialogTitle>
        <DialogContent>
          <form>
              ...
              {isCreateCourse && (
                <div className="form-group col-12 mb-15">
                  <div className="alert alert-info" role="alert">
                    As aulas poderão ser criadas e adicionadas após a criação do
                    curso.
                  </div>
                </div>
              )}
            ...
            onClick={() => {
              if (isCreateCourse) {
                handleCreateNewCourse({
                  currentTitleCourse,
                  currentDescriptionCourse
                })
              } else {
                handleEditCourse(currentIdCourse, {
                  currentTitleCourse,
                  currentDescriptionCourse
                })
              }
        ...
        <DialogContent>
          {isLoadingCoursesToAddInClass || !paginedCourseToAddInClass ? (
            ...
            <>
                ...
                <Col className="col">
                  <div className="input-group">
                    <input
                      ...
                      onChange={e =>
                        setTitleOrCodeInputCoursesToadd(e.target.value)
                      }
                    />
                    <button
                      className="btn btn-secondary btn-outline-secondary"
              ...
              {!paginedCourseToAddInClass
                ? []
                : paginedCourseToAddInClass.docs.map(course => (
