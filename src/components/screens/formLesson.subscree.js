...
const FormCourseSubscreen = ({ isEditLesson, ...props }) => {
  ...
  const { lesson, isLoadingLesson, createLesson, updateLesson /*getLesson*/ } =
    useLesson()
  const { tags, isLoadingTags, getTags } = useTag()
  const { paginedQuestions, isLoadingQuestions, getPaginedQuestions } =
    useQuestion()
  const {
  ...
  const [isLoadingQuestionsByList, setIsLoadingQuestionsByLesson] =
    useState(false)
  ...
  const [openObjectiveQuestionModal, setOpenObjectiveQuestionModal] =
    useState(false)
  const [openDiscursiveQuestionModal, setOpenDiscursiveQuestionModal] =
    useState(false)
  ...
  const handlePageObjectiveQuestions = useCallback(
    (e, numPage) => {
      e.preventDefault()
  ...
  const handlePageDiscursiveQuestions = useCallback(
    (e, numPage) => {
      e.preventDefault()
  ...
  const handleShowObjectiveQuestionModal = useCallback(
    question => {
  ...
  const handleShowDiscursiveQuestionModal = useCallback(
    question => {
  ...
  useEffect(() => {
    ...
    paginedObjectiveQuestions,
    setPageObjectiveQuestion,
    setTotalDocsObjectiveQuestion,
    setDocsPerPageObjectiveQuestion,
    setTotalPageObjectiveQuestion
  ])
  ...
  useEffect(() => {
    ...
    paginedDiscursiveQuestions,
    setPageDiscursiveQuestion,
    setTotalDocsDiscursiveQuestion,
    setDocsPerPageDiscursiveQuestion,
    setTotalPageDiscursiveQuestion
  ])
  ...
  const addQuestion = useCallback(selectedQuestion => {
    setSelectedQuestions(oldSelectedQuestions => [
  ...
  const removeQuestion = useCallback(removedQuestion => {
    setSelectedQuestions(oldSelectedQuestions =>
      oldSelectedQuestions.filter(q => q.id !== removedQuestion.id)
    )
  }, [])
  ...
  const handleTabeIndex = useCallback(
    (e, newValue) => {
    ...
    [
      page,
      pageObjectiveQuestion,
      pageDiscursiveQuestion,
      getPaginedQuestions,
      getPaginedObjectiveQuestions,
      getPaginedDiscursiveQuestions,
      getQuerys
  ...
  const handleImageUploadBefore = useCallback(() => {
    Swal.fire({
      icon: 'error',
      title:
        'Não é permitido o upload de imagens, carregue-as a partir de um link 😃'
    })
    return false
  }, [])
  ...
  const handleVideoUploadBefore = useCallback(() => {
    Swal.fire({
      icon: 'error',
      title:
        'Não é permitido o upload de vídeos, carregue-os a partir de um link 😃'
    })
    return false
  }, [])
    ...
    <>
      <Row className="mb-4">
        <Col className="col-12">
          <h1 className="m-0 h5">
            ...
            <Link to={`/professor/curso/${props.match.params.IdCourse}/aulas`}>
              {course.title}
            </Link>
            <i className="fa fa-angle-left ml-2 mr-2" />
            Criar Aula
      ...
      <Row>
        <Col className="col-12">
          ...
          <TabPanel className="mt-4" value={tabIndex} index={0}>
            <FormFilterQuestion
            ...
            <Row className="mb-4">
              <Col className="col-12">
                ...
                <table
                  className={`table table-hover table-responsive mb-0 ${
                    isLoadingQuestions ? 'd-none' : ''
                  }`}
                >
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {!paginedQuestions
                      ? []
                      : paginedQuestions.docs.map(question => {
                          return (
                            <tr key={question.id}>
                              ...
                              <td className="d-inline-flex">
                                <button
                                  ...
                                  onClick={() => handleShowModalInfo(question)}
                                >
          ...
          <TabPanel className="mt-4" value={tabIndex} index={1}>
            <Row className="mb-4">
              <Col className="col-12">
                <Load
                  className={`${!isLoadingObjectiveQuestions ? 'd-none' : ''}`}
                />
                <table
                  className={`table table-hover ${
                    isLoadingObjectiveQuestions ? 'd-none' : ''
                  }`}
                >
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {!paginedObjectiveQuestions
                      ? []
                      : paginedObjectiveQuestions.docs.map(question => (
                          <tr key={question.id}>
                            <td>{question.title}</td>
                            <td>{question.code}</td>
                            <td>
                              {arrDifficulty[parseInt(question.difficulty)]}
                            </td>
                            <td>{question.author.email}</td>
                            <td>
                              {moment(question.createdAt)
                            ...
                            <td>
                              <button
                                ...
                                onClick={() =>
                                  handleShowObjectiveQuestionModal(question)
                                }
                              >
                                <i className="fa fa-info" />
                              </button>
          ...
          <TabPanel className="mt-4" value={tabIndex} index={2}>
            <Row className="mb-4">
              <Col className="col-12">
                <Load
                  className={`${!isLoadingDiscursiveQuestions ? 'd-none' : ''}`}
                />
                <table
                  className={`table table-hover ${
                    isLoadingDiscursiveQuestions ? 'd-none' : ''
                  }`}
                >
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {!paginedDiscursiveQuestions
                      ? []
                      : paginedDiscursiveQuestions.docs.map(question => (
                          <tr key={question.id}>
                            <td>{question.title}</td>
                            <td>{question.code}</td>
                            <td>
                            ...
                            <td>
                              <button
                                ...
                                title="Ver informações"
                                onClick={() =>
                                  handleShowDiscursiveQuestionModal(question)
                                }
                              >
                                <i className="fa fa-info" />
                              </button>
      ...
      >
        ...
        <DialogContent>
          ...
          <Row>
            <Col className="col-12">
              <TableIO
                results={(modalQuestion && modalQuestion.results) || []}
              />
            </Col>
          </Row>
          ...
          <Row>
            ...
            <Col className="col-12">
              <b>Data de criação:</b>{' '}
              {modalQuestion &&
                moment(modalQuestion.createdAt)
                  .local()
                  .format('DD/MM/YYYY - HH:mm')}
            </Col>
          </Row>
                  ...
                  <Col className="col-12 mb-2" key={i}>
                    <Row className="border-0">
                      ...
                      <Col className="col-10">
                        <div
                          ref={el => (editorRef.current[i + 1] = el)}
                          className="w-100"
                        >
                          <SunEditor
                            lang="pt_br"
                            ...
                            onLoad={() => {
                              editorRef.current[i + 1].classList.add(
                                'sun-editor-wrap'
                              )
                            }}
                            setOptions={{
