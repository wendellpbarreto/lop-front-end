...
import {
  ...
  findLocalIp,
  isXml
} from '../../util/auxiliaryFunctions.util'
import AceEditorWrapper, {
  themesAceEditor
} from '../templates/aceEditorWrapper.template'
...
const QuestionSubcscreen = props => {
  ...
  const { getIconTypeQuestion, getQuestion, isLoadingQuestion, question } =
    useQuestion()
  const {
  ...
  const testCases = useMemo(
    () =>
      isTeacher() ||
      idList ||
      (test && test.classHasTest.showAllTestCases) ||
      !idClass
        ? response
  ...
  useEffect(() => {
    if (question) {
      ...
      setCharChangeNumber(
        question.questionDraft ? question.questionDraft.char_change_number : 0
      )
  ...
  const saveDraft = useCallback(
    async (showMsg = true) => {
      if (
        typeQuestion.current === 'PROGRAMMING' ||
        typeQuestion.current === 'DISCURSIVE'
      ) {
        const request = {
          answer:
            latestLanguage.current === 'blockly'
              ? getBlocklyXML(simpleWorkspace.current.workspace)
              : latestSolution.current,
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          err.response.data &&
          err.response.data.msg === "O professor recolheu a prova! :'("
        ) {
          Swal.fire({
            icon: 'error',
  ...
  const handleSubmitProgrammingQuestion = useCallback(async () => {
    ...
    const request = {
      codigo:
        language === 'blockly'
          ? getBlocklyCode(simpleWorkspace.current.workspace)
          : solution,
      ...
      <Row className="mb-4">
        <Col className="col-12">
          <h1 className="m-0 h5">
            ...
            {idList && (
              <>
                <Link to={`/${profile}/turma/${idClass}/listas`}>Listas</Link>
                <i className="fa fa-angle-left ml-2 mr-2" />
                <Link to={`/${profile}/turma/${idClass}/lista/${idList}`}>
                  {list.title}
                </Link>
                <i className="fa fa-angle-left ml-2 mr-2" />
              </>
            ...
            {idTest && (
              <>
                <Link to={`/${profile}/turma/${idClass}/provas`}>Provas</Link>
                <i className="fa fa-angle-left ml-2 mr-2" />
                <Link to={`/${profile}/turma/${idClass}/prova/${idTest}`}>
                  {test.title}
                </Link>
                <i className="fa fa-angle-left ml-2 mr-2" />
              </>
      ...
      <Row className="mb-4">
        <Col className="col-12">
          ...
          >
            <CardHead>
              <CardH2>
                <p className="d-flex m-0 font-weight-bold">
              ...
              <Row>
                <Col
                  className={`col-12 col-md-${
                    question.type === 'PROGRAMMING' ? '7' : '12'
                  }`}
                >
                  <div
                    ref={el => (editorEnunciateRef.current = el)}
                    className="w-100"
                  >
                    <SunEditor
                      lang="pt_br"
                      ...
                      onLoad={() => {
                        editorEnunciateRef.current.classList.add(
                          'sun-editor-wrap'
                        )
                      }}
                      setOptions={{
          ...
          <Row className="mb-4">
            ...
            <Col className="col-12 col-md-2">
              ...
              <select
                id="theme"
                aria-label="Tema"
                defaultValue={themeAceEditor}
                className="form-control"
                onChange={e => setThemeAceEditor(e.target.value)}
              >
                <optgroup label="Tema">
            ...
            <Col className="col-12 col-md-3">
              <label htmlFor="submit">Submeter:</label>
              <button
                id="submit"
                aria-label="Submeter"
                className={`w-100 btn btn-primary ${
                  loadingReponse && 'btn-loading'
                }`}
                onClick={() => {
              ...
              >
                <i className="fa fa-play" /> <i className="fa fa-gears mr-2" />
                Submeter
              </button>
            </Col>
            <Col className="col-5 col-md-3">
              <label htmlFor="draft">Salvar rascunho:</label>
              <button
                id="draft"
                aria-labelby="Rascunho"
                style={{ width: '100%' }}
                className={`btn btn-azure ${isSavingDraft && 'btn-loading'}`}
                onClick={saveDraft}
              >
                ...
                {language === 'blockly' ? (
                  ...
                  <AceEditorWrapper
                    ...
                    onChange={newValue => {
                      ...
                      setCharChangeNumber(
                        oldCharChangeNumber => oldCharChangeNumber + 1
                      )
                    }}
                    value={solution}
                    fontSize={14}
              ...
              <Col className="col-12">
                {question.lastSubmission.hitPercentage === 100 ? (
                  <div
                    ...
                    role="alert"
                  >
                    <i className="fa fa-smile-o mr-2" />{' '}
                    <p className="m-0">Opção correta</p>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    <p className="m-0">Opção errada</p>
                  </div>
                )}
              </Col>
            </Row>
          ...
          <Row>
            {!question.lastSubmission && (
              <Col className="col-12 col-md-3">
                ...
                <button
                  className={`w-100 btn btn-primary ${
                    isSavingSubmission && 'btn-loading'
                  }`}
                  onClick={handleSubmitObjectiveQuestion}
                >
                  <i className="fa fa-play" />{' '}
                  <i className="fa fa-gears mr-2" />
                  Submeter
                </button>
              </Col>
            )}
              ...
              <Col className="col-12">
                {question.lastSubmission.hitPercentage === null ? (
                  ...
                  >
                    <p className="m-0">
                      Nota do professor:{' '}
                      <b>{question.lastSubmission.hitPercentage}%</b>
                    </p>
                  </div>
                )}
              </Col>
